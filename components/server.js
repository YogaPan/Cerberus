...
function ensureSecure(req, res, next){
  ...
  res.redirect('https://' + req.headers.host + req.url); // express 4.x
}
...
io.on('connection', function (socket) {
  ...
  //console.log(socket.handshake.session.uid, socket.handshake.session.username);
  ...
  socket.on('disconnect', function () {
    ...
    /*connection.query('UPDATE chatmember SET exist = 0 WHERE cid=(SELECT id FROM chatroom WHERE url="'+socket.room+'") AND uid='+socket.handshake.session.uid+'', function (error, results, fields) {
      if (error) throw error;
    });*/
  });
  ...
  socket.on('new message', function (data) { // when the client emits 'new message', this listens and executes
    ...
    var mid2;
    var counter;
    //console.log(socket.handshake.session.uid,socket.room,'yoyo');
